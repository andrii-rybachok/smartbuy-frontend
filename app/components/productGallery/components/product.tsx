...
export default function Product({ product }: { product: ProductItem }) {
   ...
   const inactiveStars = [];
   const activeStars = [];
   for (let index = 0; index < 4 - Math.round(product.rating); index++) {
      inactiveStars.push(
   ...
   return (
      <div className={styles.productBlock}>
         <div className={styles.imageBlock}>
            <Image src={templateImg} alt={"Image for product"} />
         </div>
         <div className={styles.downPart}>
            <div className={styles.ratingBlock}>
               ...
               <h5 className={styles.reviews + " " + sfProLight.className}>{product.countOfReviwes} відгуків</h5>
            </div>
            <h4 className={helveticaRoman.className + " " + styles.name}>{product.name}</h4>
            <h6 className={styles.reviews + " " + sfProLight.className}>{product.shortDescription}</h6>
            <div className={styles.buyPart}>
               ...
               <div className={styles.displayFlex}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" className={isLiked?styles.activeHeart:styles.inactiveHeart} onClick={handleClick}>
                     <path
